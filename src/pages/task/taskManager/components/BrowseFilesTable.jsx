...
const deleteFile = async (record) => {
  const res = await deleteTaskFile(record.tasks[0]?.id, record.id);
  if (res.message) {
    message.info(res.message);
    return false;
  }
  ...
  return (
    <EditableProTable
      ...
      request={async (params, sort, filter) => {
        const res = await getTaskFiles(selectTask.id, {
          ...params,
          codeIndex,
        }); // 不带id 分页获取根的
      ...
      editable={{
        ...
        onSave: async (rowKey, record, row) => {
          ...
          // await waitTime(2000);
          if (record.id) {
            const res = await updateTaskFileMess(selectTask.id, record.id, {
            ...
            const res = await createTaskFileMess(selectTask.id, {
              name: record.name,
              codeIndex,
              feature: fileFeature,
              uploaderAccount: user.account,
              uploaderName: user.name,
              category: selectTask.domainClassification,
              paperNumber: record.paperNumber,
              issueDate: record.issueDate,
            });
            if (res.data) {
