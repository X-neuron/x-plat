...
const downLoadFile = async (record) => {
  const res = await getTaskFileUrl(record.tasks[0]?.id, record.id);
...
const exportword = async (task, table) => {
  console.log(table);
  const res = await downloadTaskFileListTemplate(
    'public/template/taskFileListTemplate.docx',
  );

  const templateFile = new Blob([res]);
  const handler = new TemplateHandler();
  const docx = await handler.process(templateFile, {
    zhuanxiang: task.orginateFrom,
    taskName: task.name,
    file: table.map((item) => ({
      a: item.name,
      b: item.paperNumber,
  ...
  saveFilefromBlob('task.name' + '查询文件清单' + '.docx', docx);
};
    ...
    render: (text, record, _, action) => [
      ...
      record.hash ? (
        ...
        <Upload
          ...
          action={`${paramConfig.requestBaseUrl}/file/task/${record.id}`}
        >
          <Button>上传</Button>
        </Upload>
      ),
...
const TaskFileTable = function (props) {
  // const selectTask = useRecoilValue(taskRecordAtom);
  const { record } = props;
  const taskFileTable = useRef();
  const resData = useRef();
  const [editableKeys, setEditableRowKeys] = useSafeState([]);

  useEffect(async () => {
    // console.log('filetreetable is:', taskFileTable.current);
    taskFileTable.current.reload();
  }, [record]);
  ...
  return (
    ...
    <EditableProTable
      columns={columns}
      actionRef={taskFileTable}
      // 第一个参数 params 查询表单和 params 参数的结合
      // 第一个参数中一定会有 pageSize 和  current ，这两个参数是 antd 的规范
      request={async (params, sort, filter) => {
        const res = await getTaskFiles(record.id, {
          ...params,
        }); // 不带id 分页获取根的
        if (res.message) {
          return null;
      ...
      editable={{
        ...
        onSave: async (rowKey, record, row) => {
          ...
          // await waitTime(2000);
          if (record.id) {
            const res = await updateTaskFileMess(
              // selectTask.id,
              record.tasks[0].id,
              record.id,
              {
